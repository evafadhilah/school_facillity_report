            ...
            <form action="{{ route('admin.fasilitas.update', $fasilitas) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="kategori_id"
                        ...
                        @foreach($kategori as $k)
                            <option value="{{ $k->id }}"
                                {{ old('kategori_id', $fasilitas->kategori_id) == $k->id ? 'selected' : '' }}>
                                {{ $k->nama_kategori }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="lokasi_id"
                        ...
                        @foreach($lokasis as $lokasi)
                            <option value="{{ $lokasi->id }}"
                                {{ old('lokasi_id', $fasilitas->lokasi_id) == $lokasi->id ? 'selected' : '' }}>
                                {{ $lokasi->nama_lokasi }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="kondisi"
                        ...
                        <option value="rusak_ringan" {{ old('kondisi', $fasilitas->kondisi) == 'rusak_ringan' ? 'selected' : '' }}>Rusak Ringan</option>
                        <option value="rusak_berat" {{ old('kondisi', $fasilitas->kondisi) == 'rusak_berat' ? 'selected' : '' }}>Rusak Berat</option>
                    </select>
                    @error('kondisi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
